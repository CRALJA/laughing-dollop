"""Capturing tweets which are sent from a location that is approximately the area that is Lichfield"""
    ...
    def on_success(self, data):  # when a tweet is found, it is manipulated as the code shows below
        ...
        if data['lang'] == 'en':
            ...
            if 'uber' in data['text']:
                ...
                self.tweet_dict['city'] = 'Lichfield'
                print(self.tweet_dict)
                t = self.tweet_dict
                lichfield_tweets.append(self.tweet_dict)
                lichfield_dict['tweets'] = lichfield_tweets
...
stream.statuses.filter(locations='-1.844815, 52.670945, -1.808206, 52.698918')  # this is a location box for Lichfield,
